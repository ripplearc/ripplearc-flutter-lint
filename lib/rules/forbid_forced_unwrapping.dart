...
class ForbidForcedUnwrapping extends DartLintRule {
  ...
  void run(
    ...
    context.registry.addCompilationUnit((node) {
      if (_isExcludedFile(resolver.path)) return;
  ...
  bool _isExcludedFile(String path) {
    ...
    // Exclude test files
    if (normalizedPath.contains('_test.dart') || normalizedPath.contains('/test/')) {
      return true;
    }
    // Exclude files under /testing/ with 'fake' in the path
    if (normalizedPath.contains('/testing/') && normalizedPath.contains('fake')) {
      return true;
    }
    // Exclude files generated by freezed (filename contains 'freezed')
    if (normalizedPath.contains('freezed')) {
      return true;
    }
    // Exclude files generated by json_serializable (filename ends with .g.dart)
